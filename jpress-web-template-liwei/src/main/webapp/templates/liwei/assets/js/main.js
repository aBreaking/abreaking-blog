...
function dateFormatByDatetime(datetime){
    var ts = new Date(datetime).getTime();
    return dateFormatByTimestamp(ts);
}
...
function dateFormatByTimestamp(timestamp){
    ...
    var diff = Math.round((nowTime - timestamp)/3600000);	//求出时间差--小时
...
var defaultIcon = "";
var flag = true;
function toggleIcon(newIcon,e,item){
    var s = $(e).attr("class");
    var prefix = s.split(" ")[0];
    if(newIcon!=s.split(" ")[1]){
        defaultIcon = s.split(" ")[1];
    }
    if(flag){
        $(e).attr({"class":prefix+" "+newIcon});

        flag = false;
    }else{
        $(e).attr({"class":prefix+" "+defaultIcon});
