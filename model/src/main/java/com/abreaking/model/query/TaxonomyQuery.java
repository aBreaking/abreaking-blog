...
import com.abreaking.model.ModelSorter;
...
public class TaxonomyQuery extends JBaseQuery {
	...
	//TODO
	@Deprecated
	public List<Taxonomy> getTaxonomyByLevel(){
		//firstParent : select * from taxonomy where parent_id=0;
		List<Taxonomy> list = new ArrayList<Taxonomy>();
		String firstSql = "select id,title,text,slug from taxonomy where parent_id=0 AND TYPE='category'";
		list = DAO.find(firstSql);
		//second : select * from taxonomy where parent_id = fisrtParent_id
		for (Taxonomy t : list){
			String secondSql = "select * from taxonomy where parent_id="+t.getId()+" AND TYPE='category'";
			List<Taxonomy> list2 = DAO.find(secondSql);
			t.setChildList(list2);
		}
		//thrid : //as second
		return list;
	}
